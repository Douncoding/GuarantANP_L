    ...
    private void loadOwnerLessonAndTimes(final LoadingStep step) {
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                ...
                for (Lesson lesson : mApp.openDBReadable().getLessonDao().loadAll()) {
                    if (lesson.getIid() == ownId) {
                ...
                try {
                    for (Lesson lesson : ownLessons) {
                        ...
                        Log.i(TAG, String.format(Locale.getDefault(), "%s 강의시간 동기화 개수:%d",
                                lesson.getName(), times.size()));
                        mApp.openDBWritable().getLessonTimeDao()
